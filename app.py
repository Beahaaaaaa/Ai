...
def kakao_bot():
    try:
        ...
        if not API_KEY: ans = "API Key 오류"
            ...
            payload = {"system_instruction": {"parts": [{"text": "16줄을 절대 넘기지 말고 짧고 간결하게 대답해라."}]}, "contents": [{"parts": [{"text": msg}]}]}
            res = requests.post(f"{API_URL}?key={API_KEY}", json=payload, headers={"Content-Type": "application/json"}, timeout=10)
